...
**2026-09-23 추가 확인 — 드리프트는 "새 값 추가"뿐 아니라 "구조 이동"으로도 생긴다.** `.ticket-hero`(티켓 상세 상단 카드)를 도입하며 그림자 책임이 `.container`에서 `.ticket-hero`로 옮겨갔는데, 값 자체는 이미 06/08번에 있던 것과 다른 새 그림자였음에도 "이 화면은 그림자 없음"이라는 예전 서술(풀블리드 시절 문구)이 안 고쳐진 채 남아 실제로 BLOCKED를 받았다. 즉 리팩터링/구조 변경(A 요소의 책임을 B 요소로 옮김)도 새 토큰 추가와 똑같이 문서 드리프트를 유발한다 — "값을 새로 만들었나"뿐 아니라 "어느 요소가 이 값을 담당하는지가 바뀌었나"도 체크리스트에 넣을 것.
